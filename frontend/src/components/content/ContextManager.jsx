...
const ContextManager = ({ subjectId }) => {
  ...
  const [viewContent, setViewContent] = useState(null); // { title, text } for in-app viewer
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const msg = error.response?.data?.message || error.message || 'Failed to upload context';
      toast.error(msg);
    }
  };
  ...
  // ── Determine if the context has an attached file (Cloudinary or direct URL) ──
  const hasFile = (context) => {
    if (context.fileUrl && context.fileUrl.startsWith('http')) return true;
    if (context.content && context.content.startsWith('http')) return true;
    if (context.content && context.content.includes('[Cloudinary File]')) return true;
    return false;
  };

  // ── Helper: fetch file from backend proxy with auth token ───────────────────
  // Uses native fetch() so the Axios 401 interceptor cannot trigger logout.
  const fetchFileBlob = async (contextId, download = false) => {
    const token = localStorage.getItem('accessToken');
    const baseUrl = import.meta.env.VITE_API_URL || '/api';
    const url = `${baseUrl}/context/${contextId}/proxy${download ? '?download=1' : ''}`;
  ...
  const handleShareToCommunity = async (context) => {
    try {
      const formDataToSend = new FormData();
      const validTypes = ['notes', 'report', 'ppt', 'revision_sheet', 'mock_paper'];
      const postType = validTypes.includes(context.type) ? context.type : 'notes';
      formDataToSend.append('type', postType);
      formDataToSend.append('title', context.title);

      // For file contexts, embed the Cloudinary URL reference; for text use content
      if (hasFile(context)) {
        let fileUrl = context.fileUrl;
        if (!fileUrl && context.content && context.content.includes('[Cloudinary File]')) {
          const m = context.content.match(/URL:\s*(https?:\/\/[^\s]+)/);
          if (m) fileUrl = m[1];
        }
        if (!fileUrl && context.content && context.content.startsWith('http')) {
          fileUrl = context.content;
        }
        formDataToSend.append(
          'content',
          fileUrl ? `[Cloudinary File]\nURL: ${fileUrl}` : context.content
        );
      } else {
        formDataToSend.append('content', context.content || context.title);
      }
      ...
      formDataToSend.append(
        'metadata',
        JSON.stringify({
          subject: context.metadata?.topic || context.title || 'Context',
  ...
  return (
    <div className="space-y-6">
        ...
        {contexts.map((context) => (
          <div key={context._id} className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl p-6 rounded-[1.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white dark:border-slate-800 hover:-translate-y-1 hover:shadow-lg transition-all flex flex-col h-full group">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-500 mt-1 mb-6 flex-grow line-clamp-3 leading-relaxed">
              {hasFile(context) ? '📎 File attached' : context.content}
            </p>
            <div className="flex flex-wrap gap-2 mt-auto pt-4 border-t border-slate-100 dark:border-slate-800/60 w-full">
              ...
              <button
                onClick={() => handleDownloadFile(context)}
      ...
      {/* ── In-app Text Content Viewer ─────────────────────────────────────────── */}
      {viewContent && createPortal(
        <div
          className="fixed inset-0 bg-slate-900/70 backdrop-blur-sm flex items-center justify-center z-[9999] p-4"
          onClick={() => setViewContent(null)}
        >
          <div
            className="bg-white dark:bg-slate-900 rounded-[2rem] shadow-2xl border border-slate-100 dark:border-slate-800 w-full max-w-3xl max-h-[85vh] flex flex-col"
          ...
          >
            <div className="flex items-center justify-between px-7 py-5 border-b border-slate-100 dark:border-slate-800">
              <div className="flex items-center gap-3">
                ...
                <h3 className="font-bold text-lg text-slate-800 dark:text-slate-100 truncate">{viewContent.title}</h3>
              </div>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => downloadAsMarkdown(viewContent.text, `context_${viewContent.title}`)}
                  className="flex items-center gap-1.5 text-xs font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/30 border border-emerald-100 dark:border-emerald-800 px-3 py-2 rounded-xl hover:bg-emerald-100 dark:hover:bg-emerald-900/50 transition-colors"
                >
                  <Download size={13} /> Save as MD
                </button>
                <button
                  onClick={() => setViewContent(null)}
                  className="p-2 bg-slate-100 dark:bg-slate-800 hover:bg-rose-100 dark:hover:bg-rose-900/30 text-slate-500 hover:text-rose-600 rounded-xl transition-all"
                >
                  <X size={18} />
                </button>
              </div>
            </div>
            <div className="overflow-y-auto custom-scrollbar p-7 flex-grow">
